...
class skorHelper {
	...
	function skor($start=null,$limit=10){
		global $CONFIG;
		...
		$filter='';
		$skor1=$this->apps->_request('skor1');		
		$skor2=$this->apps->_request('skor2');		
		$skor3=$this->apps->_request('skor3');		
		$skor4=$this->apps->_request('skor4');		
		$skor5=$this->apps->_request('skor5');		
		$skor6=$this->apps->_request('skor6');
		/*		
		...
		*/
		if($skor1){
				$filter .= $skor1 ? " AND `skor1`={$skor1}": "";
		}
		
		if($skor2){
				$filter .= $skor2 ? " AND `skor2`={$skor2}": "";
		}
		
		if($skor3){
				$filter .= $skor3 ? " AND `skor3`={$skor3}": "";
		}
		
		if($skor4){
				$filter .= $skor4 ? " AND `skor4`={$skor4}": "";
		}
		if($skor5){
				$filter .= $skor5 ? " AND `skor5`={$skor5}": "";
		}
		
		if($skor6){
				$filter .= $skor6 ? " AND `skor6`={$skor6}": "";
		}
		...
		$sql = "SELECT COUNT(*) as total 
				FROM ss_tebakan_skor as a inner join ss_member as b inner join ss_chapter as c on a.member_id=b.id and 
				b.chapter_id=c.id where a.week_id=(select week_id from ss_pertandingan_tebakskor where n_status=1) "; 
		
		//select count(*) as a from (select count(*) from ss_tebak_skor_detail group by id_member) ss_tebak_skor_detail where 1
		...
		$sql = "select a.*, b.*,c.*, (if(a.skor1=d.skor1 and a.skor2=d.skor2,1,0) + if(a.skor3=d.skor3 and a.skor4=d.skor4,1,0) + if(a.skor5=d.skor5 and a.skor6=d.skor6,1,0)) as total_skor,
b.name as membername		
from ss_tebakan_skor as a
inner join ss_member as b
inner join ss_chapter as c
inner join ss_pertandingan_tebakskor as d
on a.member_id=b.id and b.chapter_id=c.id and a.week_id = d.week_id
where d.n_status=1
order by total_skor DESC, a.created ASC";
		...
		foreach ($rsut as $key => $val){
			$rsut[$key]['no'] = $no++;
			$rsut[$key]['sekor1'] = $skor1;
			$rsut[$key]['sekor2'] = $skor2;
			$rsut[$key]['sekor3'] = $skor3;
			$rsut[$key]['sekor4'] = $skor4;
			$rsut[$key]['sekor5'] = $skor5;
			$rsut[$key]['sekor6'] = $skor6;
		}
		//pr($rsut[$key]['no']);die;
		$rs['status'] = true;
		$rs['result'] = $rsut;
	...
	function addskor($data){
		...
		$sql="update {$CONFIG['DATABASE'][0]['DATABASE']}.ss_pertandingan_tebakskor set skor1={$skor1},
			skor2={$skor2},skor3={$skor3},skor4={$skor4},skor5={$skor5},skor6={$skor6} where week_id={$weekid}";            
	...
	function resetpoint($idnya){
        ...
        function finishstatus($idnya){
                ...
                if($result){
                        ...
                        $sql="insert into {$CONFIG['DATABASE'][0]['DATABASE']}.ss_activity_log set type_paremeter_point=17,
	...
	function updatepoint($data){
		global $CONFIG;	
		$sql="select * from {$CONFIG['DATABASE'][0]['DATABASE']}.ss_activity_log where type_paremeter_point=17 and 				
				chapter_id='{$data['idchapter']}' and 
				user_id='{$data['idmember']}' and 
				chalangge_id='{$data['idweek']}'";
		...
		if($fetch){			
			...
			//INSERT KE ss_activity_log
			if($data['idchapter']<>'0' or $data['idmember']<>'0' or $data['idweek']<>'0'){
				$sql="insert into {$CONFIG['DATABASE'][0]['DATABASE']}.ss_activity_log set type_paremeter_point=17,n_status=1,
				chapter_id='{$data['idchapter']}',chalangge_id='{$data['idweek']}',user_id='{$data['idmember']}',date=NOW(),point='{$data['pointnya']}'";
				
				$fetch1=$this->apps->query($sql);
				
				//Update KE ss_member
				$sql="update {$CONFIG['DATABASE'][0]['DATABASE']}.ss_member set point=point+{$data['pointnya']} where id={$data['idmember']}";
	...
	function pertandingan(){
		$sql = "SELECT * FROM {$this->config['DATABASE'][0]['DATABASE']}.ss_pertandingan_tebakskor WHERE n_status=1";

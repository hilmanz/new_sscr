...
class memberHelper {
	...
	function member($start=null,$limit=10){
		...
		$sql = "SELECT COUNT(*) total 
				FROM  {$CONFIG['DATABASE'][0]['DATABASE']}.ss_member WHERE 1 {$statusquery} {$filter} "; 
		//pr($sql);exit;
		$total = $this->apps->fetch($sql);		
		//pr($sql);exit;
		if(intval($total['total'])<=$limit) $start = 0;
		
		$sql="select *,DATE_FORMAT(sm.date_register,'%d-%m-%Y %H:%i') AS date_register from  {$CONFIG['DATABASE'][0]['DATABASE']}.ss_member sm where 1 {$statusquery} {$filter}  {$order} LIMIT {$start}, {$limit} ";
		//pr($sql);exit;
	...
	function activestatus($idnya){
		...
		$sql="update  {$CONFIG['DATABASE'][0]['DATABASE']}.ss_member set n_status=1, date_register=now() where id={$idnya}";
		//pr($sql);exit;
		$fetch=$this->apps->query($sql);
                $sql="update  {$CONFIG['DATABASE'][0]['DATABASE']}.ss_akses_login set n_status=1, date=now() where user_id={$idnya}";
                //pr($sql);exit;
                $fetch=$this->apps->query($sql);
	...
	function editmember($data){
		...
		$sql1="INSERT INTO {$CONFIG['DATABASE'][0]['DATABASE']}.ss_akses_login (user_id, username, password, salt, date, role, n_status, created)
select id, username, password, '123456', date_register, 2, 1, now() from ss_member
where id = '{$data['idnya']}'
ON DUPLICATE KEY UPDATE password = VALUES(password), n_status = 1, user_id=VALUES(user_id), role=2
";
	...
	function downloadListmember(){
		...
		$sql = "select sc.name_chapter as namachapter,sm.name,sm.username,sm.ktp_sim,c.city, DATE_FORMAT(sm.date_register,'%d-%m-%Y %H:%i') AS date_register ,sm.n_status
				from  {$CONFIG['DATABASE'][0]['DATABASE']}.ss_member sm 
				left join {$CONFIG['DATABASE'][0]['DATABASE']}.ss_chapter sc on sm.chapter_id=sc.id 
				left join {$CONFIG['DATABASE'][0]['DATABASE']}.cities c on sc.kota=c.id
				where 1 AND sm.n_status !=2 {$statusquery} {$filter}  {$order}";
